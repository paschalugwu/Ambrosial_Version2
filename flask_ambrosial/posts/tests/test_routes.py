...
"""
Unit tests for Flask routes in the Flask Ambrosial application.
"""
...
class PostRoutesTestCase(unittest.TestCase):
    """Test cases for post-related routes."""
    ...
    def test_post(self):
        ...
        post = Post(
            ...
            author=self.user, image_filename='default.jpg'
        )
        db.session.add(post)
        db.session.commit()
        with self.app.test_request_context():
    ...
    def test_update_post(self):
        ...
        with self.app.test_request_context():
            with open(
                'flask_ambrosial/static/post_pics/f64d8aab113b14aa.jpg',
                'rb'
            ) as img:
                response = self.client.post(
                    url_for('posts.update_post', post_id=post.id),
                    data={
                        'title': 'Updated Post',
    ...
    def test_delete_post(self):
        ...
        with self.app.test_request_context():
            response = self.client.post(
                url_for('posts.delete_post', post_id=post.id),
                follow_redirects=True
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_get_comments(self):
        ...
        db.session.commit()
        comment = Comment(
            content='This is a test comment.', author=self.user, post=post
        )
        db.session.add(comment)
        db.session.commit()
        with self.app.test_request_context():
            response = self.client.get(
                url_for('posts.get_comments', post_id=post.id)
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_delete_comment(self):
        ...
        with self.app.test_request_context():
            response = self.client.post(
                url_for('posts.delete_comment', comment_id=comment.id),
                follow_redirects=True
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_comment(self):
        ...
        with self.app.test_request_context():
            response = self.client.post(
                url_for('posts.edit_comment', comment_id=comment.id),
                data={
    ...
    def test_delete_reply(self):
        """Test deleting a reply to a comment."""
        post = Post(
            title='Test Post', content='This is a test post.',
            author=self.user, image_filename='default.jpg'
        )
        ...
        db.session.commit()
        reply = Comment(
            content='This is a test reply.', author=self.user,
        ...
        with self.app.test_request_context():
            response = self.client.post(
                url_for('posts.delete_reply', reply_id=reply.id),
                follow_redirects=True
            )
        self.assertEqual(response.status_code, 200)
    ...
    def test_edit_reply(self):
        """Test editing a reply to a comment."""
        post = Post(
            title='Test Post', content='This is a test post.',
            author=self.user, image_filename='default.jpg'
        )
        ...
        with self.app.test_request_context():
            response = self.client.post(
                url_for('posts.edit_reply', reply_id=reply.id),
                data={
